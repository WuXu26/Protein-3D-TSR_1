...
parser.add_argument('--sample_name', '-sample', metavar='sample_name', default='t1', \
        ...
        for i in inFile:
            ...
            if args.is_chain:
                if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" \
                    ...
                    if (aa_seq is not None) and (fileName in aa_seq) and (int(re.findall(r'[0-9]+',i[22:27].rstrip())[0]) \
                            not in aa_seq[fileName])  :
                        continue
                ...
                if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" \
                    ...
                    if (aa_seq is not None) and (fileName in aa_seq) and (int(re.findall(r'[0-9]+',i[22:27].rstrip())[0]) \
                            not in aa_seq[fileName]) :
                        # print(int(re.findall(r'[0-9]+',i[22:27].rstrip())[0]),
                        #      range(int(aa_seq[fileName].rsplit('-', 1)[0]), int(aa_seq[fileName].rsplit('-', 1)[1]) + 1))
                        continue  
                    aminoacidCount.append(i[17:20])
                    aminoAcidName[counter]=int(aminoAcidLabel[i[17:20]])
                    xCord[counter]=(float(i[30:38]))
...
if __name__ == '__main__':
    ...
    if args.is_seq:
        if not 'aa' in df.columns:
            print("aa column is not present for selection of a range of aa from protein.")
            exit()
        else:
            df_seq = df[df['aa'] != "Unknown"]
            aa_seq = {}
            for k,v in dict(zip(df_seq.protein, df_seq.aa)).items():
                req_aa = []
