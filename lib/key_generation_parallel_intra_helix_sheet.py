...
parser.add_argument('--is_sheet', '-is_sheet', metavar='is_sheet', default=0, \
...
def processFiles(fileName, max_helices, is_sheet):
    ...
    for helix_in_test in range(1,max_helices):
        ...
        for i in inFile:
            ...
            # For HELIX
            if int(is_sheet) == 0:
                if df_dict[ntpath.basename(fileName)[:-4].upper()] == 'alpha':
                    if (i[0:5].rstrip())=="HELIX"  and i[19]== df_chain[ntpath.basename(fileName)[:-4].upper()] :
            ...
            # For SHEET
            if int(is_sheet) == 1:
                if df_dict[ntpath.basename(fileName)[:-4].upper()] == 'beta':
                    if (i[0:5].rstrip())=="SHEET"  and i[32]== df_chain[ntpath.basename(fileName)[:-4].upper()]:
            ...
            if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" and (i[16]=='A'or i[16]==' ')and i[17:20]!= "UNK" :
                if helix_in_test in helix_dict:
                    ...
                    print (ntpath.basename(fileName)[:-4],all_aminoacidCount,len(allkeys),sum(list(allkeys.values())))
                    return (ntpath.basename(fileName)[:-4],all_aminoacidCount,len(allkeys),sum(list(allkeys.values())))
    ...
    for value_ in allkeys:
        outFile2.writelines([str(value_),'\t', str(allkeys[value_]),'\n'])
    print ('sarika',ntpath.basename(fileName)[:-4], \
        all_aminoacidCount,\
        len(allkeys), \
        sum(list(filesDict.values())))
    return (ntpath.basename(fileName)[:-4],\
        all_aminoacidCount,\
        len(allkeys), \
        sum(list(filesDict.values())))
...
if __name__ == '__main__':
    ...
    numOfLabels = 20
    fileType = "*.pdb"  #"*.ent"#       
    aminoAcidCode=open("/work/wxx6941/TSR/Protein_Database/aminoAcidCode_lexicographic_new.txt","r") 
    df1 = pd.read_csv(os.path.join(args.path,args.sample_name,'sample_details.csv'))
    df1['protein'] = df1['protein'].str.upper()
    df_dict = dict(zip(df1.protein,df1.secondary_structure))
    df_chain = dict(zip(df1.protein,df1.chain))

    aminoAcidLabel={}
    for amino in aminoAcidCode:
        aminoAcidLabel[amino.split()[0]]=int(amino.split()[1])
    aminoAcidCode.close()
    outFolder = os.path.join(args.path, args.sample_name,"theta"+str(dTheta)+"_dist"+str(dLen), "Intra_{}".format(args.is_sheet))
    ...
    result = Parallel(n_jobs=cpu_count() - 1, verbose=10, backend="multiprocessing", batch_size="auto")\
            (delayed(processFiles)(fileName,args.max_helices, args.is_sheet) for fileName in files)
    print result
    
    df2 = pd.DataFrame(result,columns = ['protein','#amino acids','#keys', '#keys with freq'])
    df_merge = df1.merge(df2,on = 'protein')
    if int(args.is_sheet) == 1:
        df_merge.to_csv(os.path.join(args.path, args.sample_name,'sample_details_intra_sheet.csv'))
    else:
        df_merge.to_csv(os.path.join(args.path, args.sample_name,'sample_details_intra_helix.csv'))
