...
def processFiles(fileName, is_sheet):
        print( fileName)
            ...
            for i in range(0,protLen-2):
                for j in range(i+1,protLen-1):
                    for k in range(j+1, protLen):
                        ...
                        # if str(key_2) == '8459687':
                        #     print line
                        #     print seq_number
                        #     print indexOf0, indexOf1, indexOf2
...
if __name__ == '__main__':
    ...
    aminoAcidCode.close()
    outFolder = os.path.join(args.path, args.sample_name,"theta"+str(dTheta)+"_dist"+str(dLen),"Inter_{}".format(args.is_sheet))
    ...
    result = Parallel(n_jobs=cpu_count() - 1, verbose=10, backend="multiprocessing", batch_size="auto")\
            (delayed(processFiles)(fileName, args.is_sheet) for fileName in files)
    ...
    if int(args.is_sheet) == 1:
        df2.to_csv(os.path.join(args.path,args.sample_name,'sample_details_inter_sheet.csv'))
    else:
        df2.to_csv(os.path.join(args.path,args.sample_name,'sample_details_inter_helix.csv'))
