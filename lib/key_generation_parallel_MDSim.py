...
def processFiles(fileName):
        ...
        outFile2 = open((outFolder+"//" + '{}_{}_{}.keys_theta{}_dist{}'.\
            format(ntpath.basename(fileName).split('.')[0],args.sample_name, ntpath.basename(fileName).split('.')[2], str(dTheta), str(dLen)) ), "w")
        
        fileTriplets = open((outFolder +"//"+'{}_{}_{}.triplets_theta{}_dist{}'.\
            format(ntpath.basename(fileName).split('.')[0],args.sample_name, ntpath.basename(fileName).split('.')[2], str(dTheta), str(dLen))), "w")
...
if __name__ == '__main__':
    ...
    #     path = '/ssdwork/sarikak/TSR/Protien_Database/'
    # else:
    #     #path = '/home/C00219805/Research/Protien_Database/'
    #     path = '/media/c00219805/Elements/Research/Protien_Database/'
    path = args.path
    #subfolder= '/extracted_new_samples/testing/'+args.sample_name+'//'
    subfolder = args.sample_name
    fileType = "*.pdb.*"       
    aminoAcidCode=open(os.path.join(path,"aminoAcidCode_lexicographic_new.txt"),"r") 
    aminoAcidLabel={}
    ...
    outFolder = os.path.join(path, subfolder, "theta{}_dist{}".format(str(dTheta), str(dLen)))
    #Create output directory
    if not os.path.exists(outFolder):
        os.makedirs(outFolder)   

    print("Working on theta {} dist {}".format(str(dTheta),str(dLen)))
    #files=glob.glob(path+subfolder+fileType)
    files=glob.glob(os.path.join(path,subfolder,fileType))  
    ...
    df2.to_csv(os.path.join(path,subfolder,'sample_details2.csv'))
