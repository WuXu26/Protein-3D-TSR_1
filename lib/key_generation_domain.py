...
def processFiles(filePath):
        ...
        print( fileName)
        if fileName not in all_proteins:
            if args.needDistribution:
                return (fileName, \
                        0, 0, 0, 0, 0, None, None )
            else:
                return (fileName, 0, 0, 0, 0, 0)
        ...
        for i in inFile:
            ...
            if args.is_chain:
                if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" \
                    ...
                    if (fileName in aa_seq) and (int(re.findall(r'[0-9]+',i[22:27].rstrip())[0]) \
                            not in range(int(aa_seq[fileName].rsplit('-', 1)[0]), int(aa_seq[fileName].rsplit('-', 1)[1])  + 1))  :
                        continue
        ...
        if args.needDistribution:
            return (fileName, \
                    ...
                    max(allDistances) if allDistances else 0, \
                    min(allDistances) if allDistances else 0, \
                    thetaDict, \
                    lengthDict )
        else:
            return (fileName, \
...
if __name__ == '__main__':
    ...
    aminoAcidCode.close()
    df = pd.read_csv(os.path.join(args.path, args.sample_name, "sample_details.csv"), header = 0, index_col = 0,\
        dtype= {"hierarchy": str, "fold_id_short" : str, "class": str, \
            "fold_id_long": str, "fold_name": str, "protein":str, "chain":str, "aa": str}, \
            usecols =["protein", "chain", "aa"]).reset_index()
    df['protein'] = df['protein'].astype(str).str.upper()
    df['protein'] = df['protein'].astype(str).str.replace('+','')
    all_proteins = df['protein'].values
    if args.is_chain:            
        df['chain'] = df['chain'].str.upper()
        protein_chain = dict(zip(df.protein, df.chain))
    print(protein_chain)
    if args.is_seq:
        df_seq = df[df['aa'] != "Unknown"]
        aa_seq = dict(zip(df_seq.protein, df_seq.aa))
    ...
    df2 = pd.DataFrame(result,columns = ['protein','aa_count','#keys', '#keys with freq', 'max_distance', 'min_distance'])
