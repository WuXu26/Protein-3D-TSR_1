        ...
        for i in inFile:
            ...
            if args.is_chain:                 
                if(i[0:4].rstrip())=="ATOM" and (i[13:15].rstrip())=="CA" \
                    ...
                    aminoAcidName["{}_{}".format(chain_in_file, counter)]=int(aminoAcidLabel[aa])
                    xCord["{}_{}".format(chain_in_file, counter)]=(float(i[30:38]))
                    yCord["{}_{}".format(chain_in_file, counter)]=(float(i[38:46]))
                    zCord["{}_{}".format(chain_in_file, counter)]=(float(i[46:54]))
                    seq_number["{}_{}".format(chain_in_file, counter)]= chain_in_file+"_"+str(re.findall(r'[0-9]+',i[22:27].strip())[0])
                    combs_counter.append("{}_{}".format(chain_in_file, counter))
                    counter += 1
